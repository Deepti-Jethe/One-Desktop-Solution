...
router.get("/getExpiringProducts", async (req, res) => {
  try {
    const connection = await db.getConnection();
    const thirtyDaysFromNow = addDays(new Date(), 30);

    const formattedThirtyDaysFromNow = format(thirtyDaysFromNow, "yyyy-MM-dd");

    const [result] = await connection.execute(
      "SELECT * FROM products WHERE expirydate <= NOW() OR expirydate BETWEEN NOW() AND ?",
